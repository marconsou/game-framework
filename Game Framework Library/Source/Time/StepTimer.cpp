...
namespace gfl
{
	StepTimer::StepTimer() : lastTime{this->timer.GetTicks()}
	{
	...
	uint64_t StepTimer::CalculateTimeDelta()
	{
	...
	void StepTimer::FixedTimeStep(std::function<void()> update, uint64_t timeDelta)
	{
	...
	void StepTimer::CalculateFramesPerSecond(uint32_t lastFrameCount)
	{
