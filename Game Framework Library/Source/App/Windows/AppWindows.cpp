...
namespace gfl
{
	AppWindows::AppWindows(const AppConfiguration& appConfiguration, AppNotification* appNotification) :
	...
	void AppWindows::SetTitle(std::string_view title) const
	{
		SetWindowText(this->handleWindow, std::wstring{title.begin(), title.end()}.c_str());
	}

	void AppWindows::ShowMessageBox(std::string_view title, std::string_view message) const
	{
	...
	void AppWindows::Quit() const
	{
		PostQuitMessage(EXIT_SUCCESS);
	}

	int AppWindows::OnRun(std::function<void()> onRun)
	{
		this->onRun = onRun;
	...
	LRESULT CALLBACK AppWindows::WindowProcedure(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
